...
import { Trophy, CheckCircle, Clock, XCircle, Search, Filter, Mail, CreditCard } from 'lucide-react'
...
export default function WinnersPage() {
  ...
  const { data: winners, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!res.ok) throw new Error(json.error?.message || 'Failed to fetch winners')
      return json.data
    }
  })
  ...
  const handleVerifyPayment = (id: string) => {
    if (confirm('Mark this prize as PAID? This will record the time and your admin ID as the verifier.')) {
  ...
  return (
    <div className="space-y-10 pb-20">
      <header>
        <h1 className="text-3xl font-serif text-[var(--text)] mb-2">Winner Verification</h1>
        <p className="text-sm text-[var(--text-muted)]">Track matches and authorize prize payouts.</p>
      </header>
              ...
              {isLoading ? (
                Array.from({ length: 5 }).map((_, i) => (
                  <tr key={i} className="animate-pulse">
                    <td colSpan={6} className="px-6 py-6 h-16 bg-[var(--sd)]/5" />
                  </tr>
                ))
              ) : winners?.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-20 text-center text-sm text-[var(--text-muted)] italic">
                    No matching winner records found.
                  </td>
                </tr>
              ) : (
                winners?.map((result: any) => (
                  <tr key={result.id} className="group hover:bg-[var(--sd)]/10 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-10 flex items-center justify-center rounded-full bg-[var(--bg)] shadow-[var(--raised-sm)] text-[var(--green-700)] font-bold text-xs">
                          {result.user?.full_name?.substring(0, 2).toUpperCase() || result.user?.email.substring(0, 2).toUpperCase()}
                        </div>
                        <div className="flex flex-col">
                          <span className="text-sm font-semibold">{result.user?.full_name || 'No Name'}</span>
                    ...
                    <td className="px-6 py-4 text-center">
                       <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${
                        ...
                        {result.payment_status === 'paid' ? <CheckCircle size={12} /> : <Clock size={12} />}
                    ...
                    <td className="px-6 py-4 text-right">
                      {result.payment_status !== 'paid' ? (
                        ...
                        <div className="text-[10px] text-[var(--text-muted)] text-right">
                          ...
                          <p>{format(new Date(result.verified_at), 'dd/MM/yyyy HH:mm')}</p>
                        </div>
                      )}
                    </td>
                  </tr>
