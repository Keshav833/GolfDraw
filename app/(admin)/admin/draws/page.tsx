...
import { Plus, Trophy, Calendar, CheckCircle, Play, Trash2, Edit } from 'lucide-react'
...
export default function DrawsPage() {
  ...
  const { data: draws, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!res.ok) throw new Error(json.error?.message || 'Failed to fetch draws')
      return json.data
    }
  })
  ...
  const deleteMutation = useMutation({
    mutationFn: async (drawId: string) => {
      ...
      const json = await res.json()
      if (!res.ok) throw new Error(json.error?.message || 'Failed to delete draw')
      return json.data
    },
    onSuccess: () => {
  ...
  const handleDelete = (draw: any) => {
    if (confirm(`Are you sure you want to delete the draw for ${format(new Date(draw.month), 'MMMM yyyy')}?`)) {
  ...
  return (
    <div className="space-y-10 pb-20">
      <header className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-serif text-[var(--text)] mb-2">Draw Management</h1>
          <p className="text-sm text-[var(--text-muted)]">Configure, simulate, and publish monthly prize draws.</p>
        </div>
        <button 
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {[1,2,3].map(i => <div key={i} className="h-64 bg-[var(--sd)]/20 animate-pulse rounded-[32px] shadow-[var(--raised-sm)]" />)}
        </div>
      ) : error ? (
            ...
            >
              ...
              <div className="space-y-4 mb-8">
                <div className="flex justify-between text-sm">
                  <span className="text-[var(--text-muted)] font-medium">Prize Pool</span>
                  <span className="font-bold text-[var(--green-700)]">£{Number(draw.prize_pool_total).toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-[var(--text-muted)] font-medium">Winners</span>
              ...
              <div className="mt-auto grid grid-cols-2 gap-3">
                ...
                >
                  {draw.status === 'draft' ? <Edit size={14} /> : <Trophy size={14} />}
