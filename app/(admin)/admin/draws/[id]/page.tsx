...
import { Trophy, Settings, Play, CheckCircle, ChevronLeft, Loader2 } from 'lucide-react'
...
export default function DrawDetailPage() {
  ...
  const { data: draw, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!res.ok) throw new Error(json.error?.message || 'Failed to fetch draw')
      return json.data
    }
  })
  ...
  const simulateMutation = useMutation({
    mutationFn: async () => {
      ...
      const json = await res.json()
      if (!res.ok) throw new Error(json.error?.message || 'Failed to simulate draw')
      return json.data
    },
    onSuccess: (data) => {
      toast.success('Simulation completed')
      queryClient.setQueryData(['admin-draw', id], (prev: any) => ({ ...prev, ...data, status: 'simulated' }))
  ...
  if (isLoading) return <div className="flex items-center justify-center min-h-[400px]"><Loader2 className="animate-spin text-[var(--green-700)]" size={40} /></div>
        ...
        <div className="flex gap-4">
          {!isPublished && (
            ...
            >
              ...
              {simulateMutation.isPending ? 'Simulating...' : isSimulated ? 'Re-run Simulation' : 'Run Simulation'}
            </button>
          )}
      ...
      <div className="flex gap-8 border-b border-[var(--sd)] px-2">
        <button 
          ...
          className={`pb-4 px-2 text-sm font-bold transition-all relative ${
            activeTab === 'config' ? 'text-[var(--green-700)]' : 'text-[var(--text-muted)]'
          }`}
        >
          ...
          {activeTab === 'config' && <div className="absolute bottom-0 left-0 right-0 h-1 bg-[var(--green-700)] rounded-t-full shadow-[0_-2px_6px_var(--green-700)]" />}
        </button>
        <button 
          onClick={() => setActiveTab('results')}
          className={`pb-4 px-2 text-sm font-bold transition-all relative ${
            activeTab === 'results' ? 'text-[var(--green-700)]' : 'text-[var(--text-muted)]'
          }`}
        >
          <div className="flex items-center gap-2">
            <Trophy size={16} />
          ...
          {activeTab === 'results' && <div className="absolute bottom-0 left-0 right-0 h-1 bg-[var(--green-700)] rounded-t-full shadow-[0_-2px_6px_var(--green-700)]" />}
        </button>
      </div>
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            {draw.status === 'draft' ? (
              <div className="p-20 text-center space-y-4">
                <Trophy size={48} className="mx-auto text-[var(--sd)] opacity-50" />
                <p className="text-[var(--text-muted)] font-medium">Run a simulation first to see potential results.</p>
                <button 
                  onClick={() => simulateMutation.mutate()}
              ...
              <DrawSimulationPreview result={isPublished ? draw.config : draw} />
            )}
          </div>
        )}
      </div>
