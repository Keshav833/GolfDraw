...
import '@/lib/razorpay/razorpay.d';
...
export function PlanPageClient({
  ...
  const monthlyCharityAmount = (100 * charityPct) / 100;
  const monthlyPrizePoolAmount = 100 - monthlyCharityAmount;
  const yearlyCharityAmount = (999 * charityPct) / 100;
  const yearlyPrizePoolAmount = 999 - yearlyCharityAmount;
  ...
  const handleSelectPlan = async (planType: 'monthly' | 'yearly') => {
    ...
    try {
      ...
      // Step 3: Create subscription on server
      ...
      if (createData.error?.code === 'ALREADY_SUBSCRIBED') {
      ...
      const rzp = new window.Razorpay({
        ...
        prefill: {
          ...
          email: userEmail || pendingRegistration.email,
        },
        handler: async (response: any) => {
          try {
            ...
            router.replace('/dashboard?account_created=1');
            router.refresh();
          } catch (verifyErr: any) {
            ...
            <PlanCard
              ...
              breakdown={
                charityPct > 0
                  ...
                  : ['₹100.00 -> prize pool', 'Cancel anytime']
              }
              features={[
            ...
            <PlanCard
              ...
              breakdown={
                charityPct > 0
                  ...
                  : ['₹999.00 -> prize pool / year', 'Cancel anytime']
              }
              features={[
                'Entered into every monthly draw',
                charityPct > 0
