...
export async function DELETE(req: Request) {
  try {
    ...
    const { data: sub } = await supabase.from('subscriptions').select('id, razorpay_subscription_id').eq('user_id', user.id).eq('status', 'active').single();
    if (!sub || !sub.razorpay_subscription_id) return NextResponse.json({ data: null, error: { message: 'No active subscription', code: '404' } }, { status: 404 });
