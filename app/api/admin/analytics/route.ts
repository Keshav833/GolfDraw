...
export async function GET() {
  try {
    ...
    const charity_total =
      charityAlloc?.reduce((acc, entry) => acc + Number(entry.amount), 0) || 0;
    ...
    const prizes_total =
      prizesPaid?.reduce((acc, entry) => acc + Number(entry.prize_amount), 0) ||
      0;
    ...
    const draws =
      drawsData?.map((d) => {
        ...
        return {
          ...
          jackpot_winners: results.filter((r) => r.category === '5-match')
            .length,
          four_match_winners: results.filter((r) => r.category === '4-match')
            .length,
          three_match_winners: results.filter((r) => r.category === '3-match')
            .length,
        };
      }) || [];
