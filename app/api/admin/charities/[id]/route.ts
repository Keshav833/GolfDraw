...
const charitySchema = z.object({
  ...
  category: z.enum(['Golf & Sport', 'Health & Research', 'Youth & Education', 'Environment']).optional(),
...
export async function DELETE(
  ...
  try {
    ...
    if (count && count > 0) {
      return NextResponse.json(
        { data: null, error: { message: 'Cannot delete a charity with active members. Deactivate it instead.' } },
        { status: 400 }
      )
    }
