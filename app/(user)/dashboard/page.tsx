...
async function DashboardContent() {
  ...
  const subscription = (subscriptionResult.data as DashboardSubscription) ?? null;
  const scores = (scoresResult.data as Score[]) ?? [];
  const charity = normalizeCharity(charityResult.data?.charity);
  const charityPct = Number(
    charityResult.data?.charity_contribution_pct ?? user?.charity_contribution_pct ?? 0
  );
  const drawResults = ((drawsResult.data ?? []) as Array<Record<string, unknown>>).map(normalizeDrawResult);
  const totalWinnings =
    winningsResult.data?.reduce((sum, row) => sum + Number(row.prize_amount ?? 0), 0) ?? 0;
  const pending = normalizePending(pendingResult.data as Record<string, unknown> | null);
  ...
  const drawCountdown = Math.max(0, Math.ceil((drawDate.getTime() - Date.now()) / 86400000));
      ...
      >
        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-[186px_minmax(0,1fr)_214px]">
          ...
          <main className="min-w-0 space-y-4">
            <header className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-0.5 text-[11px] text-[#6a7a6a]">
                  {monthLabel} · Draw in {drawCountdown} day{drawCountdown === 1 ? '' : 's'}
                </p>
              </div>
            ...
            <section className="rounded-[20px] bg-[var(--dashboard-bg)] p-5" style={{ boxShadow: raised }}>
              <div
              ...
              >
                ...
                <svg className="absolute right-[100px] top-4 z-[1] opacity-45" width="90" height="36" viewBox="0 0 90 36" fill="none">
                  <polyline
                ...
                <div className="relative z-[1] flex items-start justify-between gap-4">
                  <div>
                    <p className="text-[11px] text-white/55">This month&apos;s jackpot</p>
                    <h2
                ...
                <div className="relative z-[1] mt-4 flex flex-wrap gap-2">
                  <HeroButton href="/scores" variant="solid" label="Submit score" />
                  <HeroButton href="/draws" variant="ghost" label="View draws" />
                </div>
              </div>
            </section>
            ...
            <section className="grid grid-cols-1 gap-3 md:grid-cols-3">
              ...
              <StatCard
                ...
                change={charityPct > 0 ? `${charityPct}% of subscription` : 'Not donating yet'}
                lineColor="#534AB7"
                icon="drop"
              />
              <StatCard
                ...
                change={isActive ? 'Subscription live' : 'Inactive subscription'}
                lineColor="#BA7517"
                icon="calendar"
              />
            </section>

            <section className="rounded-[16px] bg-[var(--dashboard-bg)] p-4" style={{ boxShadow: raisedSm }}>
              <div className="mb-3 flex items-center justify-between gap-3">
                <div className="text-[13px] font-semibold text-[#2a3a2a]">Score history</div>
                <div
              ...
              {scores.length ? (
                <>
                  <svg width="100%" height="72" viewBox="0 0 380 72" preserveAspectRatio="none">
                    <defs>
                      <linearGradient id="dashboard-score-fill" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor="#1a5e38" stopOpacity="0.12" />
                        <stop offset="100%" stopColor="#1a5e38" stopOpacity="0" />
                      </linearGradient>
                    </defs>
                    <path
                      d={chartPath}
                      ...
                      strokeLinejoin="round"
                    />
                    <path d={chartAreaPath} fill="url(#dashboard-score-fill)" />
                    {scoreSeries.map((value, index) => {
                      const x = 10 + index * (360 / Math.max(scoreSeries.length - 1, 1));
                      ...
                      return (
                        <circle
                          ...
                          fill={index === scoreSeries.length - 1 ? '#1a5e38' : 'var(--dashboard-bg)'}
                          stroke="#1a5e38"
                          strokeWidth="1.5"
                        />
                      );
                    })}
                    <rect x="336" y="2" width="44" height="16" rx="5" fill="#1a3a2a" />
                    <text
                      x="358"
                      y="13"
                      textAnchor="middle"
                ...
                >
                  No score history yet. Submit your first score after your next round.
                </div>
              )}
            </section>
...
function HeroButton({
  ...
  return (
    <Link
      ...
      style={variant === 'solid' ? { boxShadow: '2px 2px 6px rgba(0,0,0,0.15)' } : undefined}
    >
      {label}
    </Link>
  );
...
function StatCard({
  ...
  return (
    <div className="rounded-[16px] bg-[var(--dashboard-bg)] px-4 py-[14px]" style={{ boxShadow: raisedSm }}>
      <div className="mb-2 flex items-center justify-between">
        <div className="text-[10px] text-[#6a7a6a]">{label}</div>
        <div className="flex h-7 w-7 items-center justify-center rounded-[8px] bg-[var(--dashboard-bg)]" style={{ boxShadow: raisedXs }}>
      ...
      <div className="mb-1 text-[20px] font-semibold tracking-[-0.5px] text-[#2a3a2a]">{value}</div>
      <div className="text-[10px] text-[#1a5e38]">{change}</div>
    </div>
  );
}

function StatusBanner({ subscription }: { subscription: DashboardSubscription }) {
  if (subscription?.status === 'past_due') {
    return (
      <div className="mb-4 rounded-[16px] border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
        Your payment failed. <Link href="/account" className="font-semibold hover:underline">Update your payment method</Link> to stay in draws.
      </div>
    );
  }
  ...
  if (!subscription || subscription.status === 'inactive' || subscription.status === 'cancelled') {
    return (
      <div className="mb-4 rounded-[16px] border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">
        Your subscription is inactive. <Link href="/account" className="font-semibold hover:underline">Resubscribe</Link> to enter next month&apos;s draw.
      </div>
    );
  }
...
function normalizeDrawResult(value: Record<string, unknown>): DrawResult {
  return {
    id: String(value.id),
    match_category: (value.match_category as DrawResult['match_category']) ?? null,
    prize_amount: value.prize_amount == null ? null : Number(value.prize_amount),
    payment_status: (value.payment_status as DrawResult['payment_status']) ?? null,
...
function normalizePending(value: Record<string, unknown> | null): PendingWinnings {
  if (!value) return null;
  const draw = Array.isArray(value.draw) ? value.draw[0] : value.draw;
  return {
    id: String(value.id),
    prize_amount: Number(value.prize_amount ?? 0),
    match_category: (value.match_category as '3-match' | '4-match' | '5-match') ?? '3-match',
    draw: draw && typeof draw === 'object' ? { month: String((draw as Record<string, unknown>).month ?? '') } : null,
  };
}

function buildScoreSeries(scores: Score[]) {
  const values = scores.slice(0, 6).map((score) => score.value).reverse();
...
function buildAreaPath(values: number[]) {
  ...
  const lastX = 10 + (values.length - 1) * (360 / Math.max(values.length - 1, 1));
