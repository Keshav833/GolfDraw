...
export function WinningsWidget({
    ...
    return (
      ...
      >
        ...
        <p className="mt-1 text-[11px] text-[#6c5310]">
          {pendingResult.match_category.replace('-', ' ')} in {pendingResult.draw?.month ?? 'recent'} draw
        </p>
        <p className="mt-2 text-[11px] text-[#6c5310]">Upload your scorecard to claim your prize.</p>
        <Link href="/draws" className="mt-3 inline-flex text-[11px] font-semibold text-[#4e3a00] hover:underline">
          Upload proof now →
        </Link>
        <p className="mt-3 text-[10px] text-[#6c5310]">Total earned: £{totalPaid.toFixed(2)}</p>
      </div>
    );
  }
  ...
  if (totalPaid > 0) {
    return (
      <div className="rounded-[16px] bg-[var(--dashboard-bg)] p-4" style={{ boxShadow: raisedSm }}>
        ...
        <h3 className="mt-2 text-2xl font-bold text-[#0f3d2e]">£{totalPaid.toFixed(2)}</h3>
        <p className="mt-1 text-[11px] text-[#6a7a6a]">Across all verified draws</p>
        <Link href="/draws" className="mt-3 inline-flex text-[11px] font-semibold text-[#1a5e38] hover:underline">
          View history →
        </Link>
      </div>
    );
  ...
  return (
    <div className="rounded-[16px] bg-[var(--dashboard-bg)] p-4" style={{ boxShadow: raisedSm }}>
      ...
      <p className="mt-1 text-[11px] text-[#6a7a6a]">Win by matching the monthly draw number with your scores.</p>
      <Link href="/draws" className="mt-3 inline-flex text-[11px] font-semibold text-[#1a5e38] hover:underline">
        How draws work →
      </Link>
    </div>
