...
export default function CharityForm({ charity, onSave, onClose }: CharityFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = charity ? `/api/admin/charities/${charity.id}` : '/api/admin/charities'
      ...
      if (json.data) {
        toast.success(`Charity ${charity ? 'updated' : 'created'} successfully`)
        onSave(json.data)
      } else {
      ...
      <div className="relative w-full max-w-xl bg-[var(--bg)] rounded-[32px] shadow-[var(--raised-md)] overflow-hidden animate-in fade-in zoom-in duration-300">
        <div className="p-8 space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-serif">{charity ? 'Edit' : 'Add'} Charity</h2>
            <button 
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-4">
              <div className="space-y-2">
                <label className="text-[10px] uppercase font-bold text-[var(--text-muted)] tracking-wider ml-1">Name</label>
                <input 
                  type="text" 
                  required
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-[10px] uppercase font-bold text-[var(--text-muted)] tracking-wider ml-1">Category</label>
                  <select 
                ...
                <div className="space-y-2">
                  <label className="text-[10px] uppercase font-bold text-[var(--text-muted)] tracking-wider ml-1">Country</label>
                  <input 
                    type="text" 
                    required
                    className="w-full px-4 py-3 rounded-xl bg-[var(--bg)] shadow-[var(--inset-sm)] focus:outline-none text-sm placeholder:opacity-30"
            ...
            <div className="pt-4 flex items-center justify-between">
              <div className="flex items-center gap-2 group cursor-help">
                ...
                />
                <label htmlFor="is_active" className="text-sm font-bold cursor-pointer">Set as Active</label>
              </div>
