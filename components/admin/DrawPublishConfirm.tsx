...
export default function DrawPublishConfirm({ drawId, winnerCount, rolloverAmount, onClose }: DrawPublishConfirmProps) {
  ...
  const handlePublish = async () => {
    ...
    try {
      const res = await fetch('/api/draws/publish', { // Note: using the existing API path
        method: 'POST',
        ...
        <div className="p-8 space-y-6">
          <div className="flex flex-col items-center text-center space-y-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm text-[var(--text-muted)]">This will make the results official and permanent.</p>
            </div>
          </div>

          <div className="p-4 rounded-2xl bg-[var(--bg)] shadow-[var(--inset-sm)] space-y-3">
            <h3 className="text-[10px] font-bold uppercase tracking-wider text-[var(--text-muted)]">Summary of Actions</h3>
