...
export default function DrawConfigForm({ draw, onSave }: DrawConfigFormProps) {
  const [month, setMonth] = useState(
    draw?.month || format(addMonths(new Date(), 1), 'yyyy-MM')
  );
  const [mode, setMode] = useState<'random' | 'algorithmic'>(
    draw?.mode || 'random'
  );
  const [prizePoolOverride, setPrizePoolOverride] = useState<number>(
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 group cursor-help">
            ...
            <span className="text-[10px] text-[var(--text-muted)] opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
              {mode === 'random'
                ? 'Fair lottery — equal chance'
                : 'Score-weighted — average-centric odds'}
            </span>
          </div>
        </div>
        <div className="p-1 rounded-2xl bg-[var(--bg)] shadow-[var(--inset-sm)] flex">
          <button
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="p-4 rounded-xl bg-[var(--bg)] shadow-[var(--raised-sm)]">
            ...
            <p className="text-lg font-bold">
              £{prepData?.calculated?.toFixed(2) || '0.00'}
            </p>
          </div>
          <div className="p-4 rounded-xl bg-[var(--bg)] shadow-[var(--raised-sm)] text-amber-600">
            ...
            <p className="text-lg font-bold">
              £{prepData?.rollover?.toFixed(2) || '0.00'}
            </p>
          </div>
        </div>
        ...
        <div className="p-6 rounded-2xl bg-[var(--bg)] shadow-[var(--inset-sm)]">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--green-700)] font-bold">
                £
              </span>
              <input
                type="number"
          ...
          <div className="space-y-2 border-t border-[var(--sd)] pt-4">
            <PrizeSplit
              label="5-match (40%)"
              amount={prizePoolOverride * 0.4}
            />
            <PrizeSplit
              label="4-match (35%)"
              amount={prizePoolOverride * 0.35}
            />
            <PrizeSplit
              label="3-match (25%)"
              amount={prizePoolOverride * 0.25}
            />
          </div>
        </div>
      </section>
      ...
      <section className="p-5 rounded-2xl bg-[var(--bg)] shadow-[var(--raised-sm)] flex items-center justify-between">
        <div>
          <p className="text-sm font-bold">
            {prepData?.eligible_count || 0} Eligible Users
          </p>
          <p className="text-[10px] text-[var(--text-muted)]">
            Active subscribers with at least 1 score
          </p>
        </div>
        {loading && (
          <RefreshCw
            className="animate-spin text-[var(--green-700)]"
            size={18}
          />
        )}
      </section>
