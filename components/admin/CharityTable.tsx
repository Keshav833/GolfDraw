...
export default function CharityTable({
  ...
  const handleToggleActive = async (charity: Charity) => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(
          `Charity ${!charity.is_active ? 'activated' : 'deactivated'}`
        );
        onRefresh();
      } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="px-4 py-3 rounded-2xl bg-amber-50 border border-amber-100 text-[11px] text-amber-700 italic">
        <strong>Note:</strong> Existing members are unaffected by deactivation.
        New members cannot select an inactive charity.
      </div>
    </div>
  );
}
